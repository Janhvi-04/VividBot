...
import { ArrowLeft, Terminal, BookOpen, Send, Sparkles, Loader2 } from "lucide-react";
...
function CodingSnippetsPage() {
  ...
  const fetchAIQuestion = async (personaType) => {
    setLoading(true);
    try {
      const res = await apiFetch('/api/snippets/generate-question', {
        method: "POST",
  ...
  const handleSendMessage = async (e) => {
    ...
    setChatLoading(true);
    try {
      const res = await apiFetch('/api/snippets/respond', {
        method: "POST",
        body: JSON.stringify({ 
  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden bg-slate-950 flex flex-col justify-between p-4 sm:p-6">
      <div
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto flex items-center justify-between">
        ...
        <span className="text-xs sm:text-sm font-bold text-white tracking-widest uppercase drop-shadow-md text-center">
      ...
      {!userPersona ? (
        <div className="relative z-20 flex-1 flex items-center justify-center w-full max-w-2xl mx-auto my-auto px-2">
          <div className="grid gap-4 w-full">
            <button
              onClick={() => setUserPersona("non-coder")}
              className="p-5 sm:p-6 rounded-2xl border border-white/20 hover:border-emerald-500/60 transition text-left cursor-pointer group shadow-xl bg-white/5 backdrop-blur-md flex flex-col justify-between"
            >
              <div>
        ...
        <div className="relative z-10 flex-1 w-full max-w-3xl mx-auto flex flex-col justify-between py-4 space-y-4">
          <div className="flex-1 overflow-y-auto space-y-4 pr-1 max-h-[65vh]">
            <div className="rounded-2xl border border-white/20 bg-white/10 backdrop-blur-md p-4 sm:p-6 text-white shadow-lg">
              ...
              {loading ? (
                ...
                <div>
                  ...
                  <span className="text-[10px] px-2.5 py-1 rounded-full bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 font-medium inline-block">
            ...
            {conversation.map((msg, idx) => (
              <div
                key={idx}
                className={`p-3 sm:p-4 rounded-2xl max-w-[85%] text-xs sm:text-sm backdrop-blur-md ${
                  msg.sender === "user"
                    ? "ml-auto bg-emerald-600/30 border border-emerald-500/40 text-white"
                    : "mr-auto bg-white/10 border border-white/20 text-slate-200"
                }`}
              >
                {formatText(msg.text)}
              </div>
            ...
            {chatLoading && (
              <div className="mr-auto p-3 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 text-slate-300 text-xs sm:text-sm flex items-center gap-2">
          ...
          <div className="w-full">
            <form onSubmit={handleSendMessage} className="relative flex items-center">
              <input
                ...
                className="w-full rounded-xl border border-white/20 bg-white/10 backdrop-blur-md px-4 py-3 pr-12 text-xs sm:text-sm text-white focus:outline-none focus:border-emerald-500/65 placeholder:text-slate-400 shadow-inner"
              />
              <button
                type="submit"
                className="absolute right-2 p-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white transition cursor-pointer shadow-sm"
              >
