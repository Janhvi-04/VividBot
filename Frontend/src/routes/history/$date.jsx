    ...
    <main className="relative min-h-screen bg-background p-6 sm:p-10">
      ...
      <div className="relative mx-auto max-w-4xl">
        ...
        <div className="rounded-3xl bg-sidebar/90 p-6 backdrop-blur-[2px]">
          <h1 className="font-display text-2xl uppercase tracking-wide text-sidebar-foreground mb-6">
            Activities on {date}
          </h1>
          <div className="space-y-3">
            {activities.length > 0 ? (
              activities.map((act, idx) => {
                ...
                return (
                  <div key={idx} className="rounded-2xl bg-sidebar-accent/30 p-4 text-sidebar-foreground flex items-start justify-between gap-4">
                    <div>
                      <p className="font-bold text-base">{act.title}</p>
                      <span className="text-xs opacity-75 uppercase tracking-wider">{act.activityType}</span>
                    </div>
                    <span className="text-xs font-mono opacity-80 whitespace-nowrap bg-sidebar-accent/50 px-2.5 py-1 rounded-lg">
                      {timeString}
                    </span>
                  </div>
                );
              ...
              <p className="text-sidebar-foreground/70">No actions recorded for this date.</p>
            )}
          </div>
        </div>
      </div>
