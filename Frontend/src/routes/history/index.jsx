      ...
      <div className="relative mx-auto w-full max-w-4xl z-10 flex-1 flex flex-col justify-center py-4">
        ...
        <div className="rounded-3xl bg-sidebar/90 p-5 sm:p-8 backdrop-blur-[2px] shadow-xl w-full">
          <div className="flex items-center gap-3 text-sidebar-foreground mb-6">
            <Clock className="h-6 w-6 flex-shrink-0" strokeWidth={1.25} />
            <h1 className="font-display text-xl sm:text-2xl uppercase tracking-wide truncate">Activity History</h1>
          </div>
            ...
            {uniqueDates.length > 0 ? (
              uniqueDates.map((dateStr) => (
                <div
                  ...
                  className="flex items-center justify-between rounded-2xl bg-sidebar-accent/40 p-4 cursor-pointer transition hover:bg-sidebar-accent text-sidebar-foreground shadow-sm"
                >
                  <span className="font-body text-base sm:text-lg font-medium">{dateStr}</span>
                  <span className="text-xs uppercase tracking-wider opacity-75 flex-shrink-0 ml-2">View →</span>
                </div>
              ))
            ) : (
              <p className="text-sidebar-foreground/70 italic text-sm">No past activity logs found.</p>
            )}
          </div>
        </div>
      </div>
