...
function LogicPuzzlesPage() {
  ...
  const [messages,setMessages]=useState({"Daily Riddle":[],"Pattern Sequence":[],"Equation":[],"Word Ladder":[]})
  const puzzleOptions = [
    { name: "Daily Riddle"},
  ...
  const fetchNewPuzzle=async(category)=>{
    ...
    try {
        ...
        if(data.success) {
            ...
            // Log activity when puzzle is generated
      ...
      <div className="relative z-10 flex min-h-screen flex-col">
        ...
        <div className="p-3 sm:p-4 md:p-6 grid grid-cols-3 items-center">
          <div className="flex justify-start">
            ...
            >
              <ArrowLeft className=" h-6 w-6 sm:h-4 sm:w-4" />
          </button>
          </div>
          <div className="flex justify-center">
            <span className="text-xs text-center font-semibold text-gray-600 px-3 py-1 rounded-full backdrop-blur-sm">{selectedOption.toUpperCase()}</span>
          </div>
          </div>

        {/* Main Chat Area */}
        <div className="flex-1 px-3 pb-3 sm:px-4 sm:pb-4 md:px-6 md:pb-6">
          <div className="mx-auto max-w-2xl">
            {/* Chat Bubbles */}
            {!hasStartedCategory ? (
              <div className="mt-30 text-center bg-transparent p-8 space-y-4">
                ...
                <button
                  ...
                  disabled={loading}
                  className="inline-flex items-center gap-2 rounded-full bg-[#5B9BD5] px-6 py-3 text-xs sm:text-sm font-semibold text-white shadow-md hover:bg-[#4A8AC7] transition disabled:opacity-50"
                >
            ...
            <div className="space-y-3 sm:space-y-4">
              ...
              {loading && (
                <div className="flex justify-start">
                    <div className="bg-transparent text-gray-500 rounded-2xl rounded-bl-none px-4 py-3 text-xs italic shadow-sm">
              ...
              {isEvaluating && (
                <div className="flex justify-start">
                    <div className="bg-transparent text-gray-500 rounded-2xl rounded-bl-none px-4 py-3 text-xs italic shadow-sm animate-pulse">
            ...
            <div className="flex flex-col gap-2 sm:gap-3">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-full border-0 bg-white px-4 py-2.5 sm:px-6 sm:py-4 pr-14 sm:pr-20 text-xs sm:text-sm shadow-md focus:outline-none focus:ring-2 focus:ring-[#5B9BD5]/50 placeholder:text-gray-400"
                />
                <button 
                  onClick={handleSend}
                  disabled={loading}
                  className="absolute cursor-pointer right-2 top-1/2 -translate-y-1/2 rounded-full bg-[#5B9BD5] p-1.5 sm:p-2.5 text-white hover:bg-[#4A8AC7] transition shadow-sm disabled:opacity-50"
                >
              ...
              <button 
                ...
                className="flex items-center justify-center cursor-pointer gap-2 rounded-full bg-white px-4 py-2.5 sm:px-6 sm:py-4 text-xs sm:text-sm font-medium text-gray-700 shadow-md hover:bg-gray-50 transition disabled:opacity-50"
              >
                <Lightbulb className={`h-3 w-3 sm:h-4 sm:w-4 transition-colors ${hintUsed?"text-yellow-500 fill-yellow-400":"text-gray-500"}`} />
        ...
        <div className="border-t border-white/30 bg-white/40 backdrop-blur-md">
          <div className="mx-auto max-w-2xl px-2 sm:px-4 py-2 sm:py-4">
            <div className="flex items-center justify-around gap-1 sm:gap-2">
              {puzzleOptions.map((item) => {
                ...
                <button
                  ...
                  className={`flex-1 rounded-full px-1.5 py-1.5 sm:px-3 sm:py-2.5 text-[9px] sm:text-xs font-medium transition ${
                    isSelected ?
